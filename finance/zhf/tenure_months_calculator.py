...
CONFIG = {
    ...
    # 是否合并"总在职天数"列的单元格：True = 按人合并并垂直居中（默认），False = 按行展开不合并
    "merge_total_days": True,
}
# ==================================================================
...
def process(input_file: str, output_file: str, **options):
    ...
    **options   : 覆盖 CONFIG 默认值
    ...
    # 新增"总在职天数"列
    if cfg.get("merge_total_days", True):
        # 合并模式：按人（姓名 + 身份证）汇总所有展开行的在职天数
    ...
    merge_flag = cfg.get("merge_total_days", True)
    if merge_flag:
        print(f"   已按人合并：仅"'总在职天数'"列（垂直居中）")
    else:
        print(f"   未合并：总在职天数列按行展开（每行 = 该行在职天数）")
